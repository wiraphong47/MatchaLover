...
const choices = [
  ...
  },
  {
    label: "ทำมัทฉะลาเต้",
    key: "latte",
    title: "สำหรับมัทฉะลาเต้ แนะนำ",
    names: ["Premium Blend", "Okumidori Matcha", "Ujihikari Matcha"],
  },
  {
    label: "ทำขนมหรือเครื่องดื่ม",
    key: "baking",
    title: "สำหรับขนมและเครื่องดื่ม แนะนำ",
    names: ["Culinary Grade"],
  },
];
...
export default function RecommendationQuiz({ products, onAdd, onView }) {
  ...
  const recommendations = choice
    ? products.filter((product) => choice.names.includes(product.name))
    : [];
  return (
    <Box
      component="section"
    ...
    >
      ...
      >
        <Box>
          ...
          <Typography
            variant="h3"
            sx={{ fontSize: { xs: 29, md: 35 }, mt: 0.7 }}
          >
      ...
      {choice && (
        ...
        >
          <Box>
            <Typography
              sx={{ color: "#d6dfac", fontSize: 13, fontWeight: 700 }}
            >
              {choice.title}
            </Typography>
          ...
          <Button
            onClick={() => onAdd(recommendations[0])}
            variant="contained"
            disableElevation
            sx={{ bgcolor: "#efe2bd", color: "#183b2a", flexShrink: 0 }}
          >
            ใส่ตัวแนะนำลงตะกร้า
          </Button>
        </Box>
